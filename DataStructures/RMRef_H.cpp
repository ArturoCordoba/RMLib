...
void RMRef_H::operator = (RMRef_H &ref) {
    //Se forma el nuevo key de la copia
    std::string* stringKey = (std::string*) malloc(sizeof(std::string));
    stringKey->append(ref.getKey(), strlen(ref.getKey()));
    stringKey->append("-copy", strlen("-copy"));
    key = (char*) stringKey->c_str(); //Se transforma el key a char* y se guarda
    value = ref.getValue(); //Se guarda el value
    value_size = ref.getValue_size(); //Se guarda el value_size

    RMLib::getInstance()->rm_new(key, value, value_size);
}